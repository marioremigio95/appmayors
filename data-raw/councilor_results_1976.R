## code to prepare `councilor_results_1976` dataset goes here

councilor_results_1976 <-
  map_df(.x = list.files("data-raw", pattern = "results_76"),
         .f = ~read_csv2(paste0("data-raw/", .))) %>%
  janitor::clean_names() %>%
  rename(name_mun_1970 = municipios,
         uf_1970 = sigla,
         year = ano) %>%
...
usethis::use_data(councilor_results_1976, overwrite = TRUE)
